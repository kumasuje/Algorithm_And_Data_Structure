...
//				Arrays.fill(inOrder, "UNUSED");
		...
		/* Implementing Our Algo on Graph we have build using workingArray , parrentMatrix and childMatrix*/
			...
			pathWithEdge.put(workingArray[0], new ArrayList<String>());
				...
				for(int i =1; i< inOrdered.size();i++)
				{
					...
					for(int j =0; j < tempSiz; j++)
					{
						...
						else
						{
							...
							if(temp2.length > 1)
							{
								addEdge(tempGraphSoFar,tempIncoming,graphSoFar,pathWithEdge,edgeMatrix);
							}
				...
				System.out.println(" Total Possible sub set PathWithEdge =  "+pathWithEdge.size());

				System.out.println(" sub-sets with edge ");
				for(Map.Entry<String,ArrayList<String>> entry : pathWithEdge.entrySet())	
				{
					System.out.println("Path containing node  = "+entry.getKey()+" with edges = "+entry.getValue());
				}
	...
	public static void addEdge(String tempGraphSoFar,String tempIncoming,ArrayList<String> graphSoFar,
